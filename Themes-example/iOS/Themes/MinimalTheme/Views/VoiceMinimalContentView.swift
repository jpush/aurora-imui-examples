//
//  VoiceMinimalContentView.swift
//  Themes
//
//  Created by oshumini on 2017/12/28.
...
public class VoiceMinimalContentView: UIView, IMUIMessageContentViewProtocol {
  static var outGoingVoiceDurationColor = UIColor(netHex: 0x222222)
  static var inComingVoiceDurationColor = UIColor(netHex: 0x222222)
  ...
  func resetVoiceImage() {
    if (message?.isOutGoing)! {
      self.voiceImg.image = UIImage.init(named: "minimal_outgoing_voice_3")
    } else {
      self.voiceImg.image = UIImage.init(named: "minimal_incoming_voice_3")
    }
  }
  
  func setImage(with index:Int) {
    if (message?.isOutGoing)! {
      self.voiceImg.image = UIImage.init(named: "minimal_outgoing_voice_\(index)")
    } else {
      self.voiceImg.image = UIImage.init(named: "minimal_incoming_voice_\(index)")
    }
  }
  ...
  func layoutToVoice(isOutGoing: Bool) {
    ...
    if isOutGoing {
      ...
      voiceDuration.textColor = VoiceMinimalContentView.outGoingVoiceDurationColor
    } else {
      self.voiceImg.image = UIImage.init(named: "minimal_incoming_voice_3")
      ...
      voiceDuration.textColor = VoiceMinimalContentView.inComingVoiceDurationColor
    }
  }
}
