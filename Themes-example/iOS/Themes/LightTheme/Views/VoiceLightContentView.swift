...
public class VoiceLightContentView: UIView, IMUIMessageContentViewProtocol {
  static var outGoingVoiceDurationColor = UIColor.white
  static var inComingVoiceDurationColor = UIColor(netHex: 0x8D8E8E)
  static var shadowWidth: CGFloat = 2.0
  ...
  public func layoutContentView(message: IMUIMessageModelProtocol) {
    ...
    IMUIAudioPlayerHelper.sharedInstance.renewProgressCallback(message.msgId) { (id, power,currendTime, duration) in
  ...
  func resetVoiceImage() {
    if (message?.isOutGoing)! {
      self.voiceImg.image = UIImage.init(named: "light_outgoing_voice_3")
    } else {
      self.voiceImg.image = UIImage.init(named: "light_incoming_voice_3")
    }
  }
  
  func setImage(with index:Int) {
    if (message?.isOutGoing)! {
      self.voiceImg.image = UIImage.init(named: "light_outgoing_voice_\(index)")
    } else {
      self.voiceImg.image = UIImage.init(named: "light_incoming_voice_\(index)")
    }
  }
  
  func layoutToVoice(isOutGoing: Bool) {
    let shadowidth = VoiceLightContentView.shadowWidth
    if isOutGoing {
      self.voiceImg.image = UIImage.init(named: "light_outgoing_voice_3")
      self.voiceImg.frame = CGRect(x: 0, y: 0, width: 12, height: 16)
      self.voiceImg.center = CGPoint(x: frame.width - 20, y: (frame.height - shadowidth)/2 )
      self.voiceDuration.center = CGPoint(x: 30, y: (frame.height - shadowidth)/2)
      voiceDuration.textAlignment = .left
      voiceDuration.textColor = VoiceLightContentView.outGoingVoiceDurationColor
    } else {
      self.voiceImg.image = UIImage.init(named: "light_incoming_voice_3")
      
      self.voiceImg.frame = CGRect(x: 0, y: 0, width: 12, height: 16)
      self.voiceImg.center = CGPoint(x: 20, y: (frame.height - shadowidth)/2)
      self.voiceDuration.center = CGPoint(x: frame.width - 30, y: (frame.height - shadowidth)/2)
      voiceDuration.textAlignment = .right
      voiceDuration.textColor = VoiceLightContentView.inComingVoiceDurationColor
    }
  }
}
